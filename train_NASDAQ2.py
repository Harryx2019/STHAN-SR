...
def get_batch(eod_data, mask_data, gt_data, price_data,seq_len,steps,offset):
...
def train(data_path,market_name,tickers_fname,rel_data_path,parameters):
    ...
    # eod_data[k折 * 股票数 * 每折天数 * 特征数]  每折天数：[504,756,1008,1245]
                ...
                for k in range(4):
                    ...
                    for j in tqdm(range(train_index[k+1] - parameters['seq'] - steps +1)):
                        ...
                        emb_batch, mask_batch, price_batch, gt_batch = \
                            get_batch(eod_data_k, mask_data_k, gt_data_k, price_data_k,seq_len,steps,batch_offsets_k[j])
                    ...
                    with torch.no_grad():
                        ...
                        for cur_offset in range(valid_index_begin,valid_index_end):
                            emb_batch, mask_batch, price_batch, gt_batch = \
                                get_batch(eod_data_k, mask_data_k, gt_data_k, price_data_k,seq_len,steps,cur_offset)
